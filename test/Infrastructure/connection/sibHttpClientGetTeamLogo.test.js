...
describe('sibHttpClientGetTeamLogo', () => {
	...
	it('should construct the correct URL with token', async () => {
		...
		const result = await sibHttpClientGetTeamLogo(mockBaseUrl, mockTeamId, mockToken, mockDeviceId)

		expect(http.get).toHaveBeenCalledWith(expect.stringContaining('/api/team-logo/'), expect.any(Function))
		expect(http.get).toHaveBeenCalledWith(expect.stringContaining(`${mockTeamId}`), expect.any(Function))
	...
	it('should include deviceId as query parameter in the URL', async () => {
		...
		await sibHttpClientGetTeamLogo(mockBaseUrl, mockTeamId, mockToken, mockDeviceId)
	...
	it('should reject on HTTP error status code', async () => {
		...
		await expect(sibHttpClientGetTeamLogo(mockBaseUrl, mockTeamId, mockToken, mockDeviceId)).rejects.toThrow(
			'HTTP Error 500',
		)
	})
	...
	it('should handle different team IDs', async () => {
		...
		const result = await sibHttpClientGetTeamLogo(mockBaseUrl, teamId, mockToken, mockDeviceId)

		expect(http.get).toHaveBeenCalledWith(expect.stringContaining(`/api/team-logo/${teamId}/`), expect.any(Function))
