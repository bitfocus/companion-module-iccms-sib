import { sibHttpClientRundownSelect } from '../../../src/infrastructure/connection/sibHttpClient.js'
...
describe('sibHttpClientRundownSelect', () => {
	...
	it('should construct the correct URL with token', async () => {
		...
		const result = await sibHttpClientRundownSelect(mockBaseUrl, mockRundownId, mockToken, mockDeviceId)

		expect(http.get).toHaveBeenCalledWith(expect.stringContaining('/api/rundown/select-rundown/'), expect.any(Function))
	...
	it('should include deviceId as query parameter in the URL', async () => {
		...
		await sibHttpClientRundownSelect(mockBaseUrl, mockRundownId, mockToken, mockDeviceId)
	...
	it('should reject on HTTP error status code', async () => {
		...
		await expect(sibHttpClientRundownSelect(mockBaseUrl, mockRundownId, mockToken, mockDeviceId)).rejects.toThrow(
			'HTTP Error 500',
		)
	})
