...
describe('sibHttpClientRundownSelectNextItem', () => {
	...
	it('should construct the correct URL with token', async () => {
		...
		const result = await sibHttpClientRundownSelectNextItem(mockBaseUrl, mockToken, mockDeviceId)

		expect(http.get).toHaveBeenCalledWith(expect.stringContaining('/api/rundown/select-next/'), expect.any(Function))
	...
	it('should construct the correct URL without token', async () => {
		...
		await sibHttpClientRundownSelectNextItem(
			mockBaseUrl,
			null, // No token
			mockDeviceId,
		)
		...
		expect(callUrl).toMatch(/\/api\/rundown\/select-next\/\?/)
	})
	...
	it('should include deviceId as query parameter in the URL', async () => {
		...
		await sibHttpClientRundownSelectNextItem(mockBaseUrl, mockToken, mockDeviceId)
	...
	it('should reject on HTTP error status code', async () => {
		...
		await expect(sibHttpClientRundownSelectNextItem(mockBaseUrl, mockToken, mockDeviceId)).rejects.toThrow(
			'HTTP Error 500',
		)
	})
