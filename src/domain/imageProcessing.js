		...
		for (let y = 0; y < scaledH; y++) {
			...
			for (let x = 0; x < scaledW; x++) {
				...
				if (outA > 0) {
					...
					out.data[outIdx + 1] = Math.round(
						(src.data[srcIdx + 1] * srcA + out.data[outIdx + 1] * dstA * (1 - srcA)) / outA,
					)
					out.data[outIdx + 2] = Math.round(
						(src.data[srcIdx + 2] * srcA + out.data[outIdx + 2] * dstA * (1 - srcA)) / outA,
					)
